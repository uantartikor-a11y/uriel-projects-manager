        ...
        with col_dl2:
            def generate_general_pdf(df):
                ...
                for _, row in df.iterrows():
                    val_c = row.get("סכום חוזה (ללא מע\"מ)", 0)
                    val_e = row.get("סה\"כ הוצאות", 0)
                    val_p = row.get("רווח נקי", 0)
                    
                    contract_val_str = f"{val_c:,.0f}"
                    exp_val_str = f"{val_e:,.0f}"
                    profit_val_str = f"{val_p:,.0f}"
